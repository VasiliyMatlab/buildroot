...
VVORONIN_BOARD_DIR="$(dirname "$0")"
DEFCONFIG_NAME="$(basename "$2")"
README_FILES="${VVORONIN_BOARD_DIR}/*/readme.txt"
START_QEMU_SCRIPT="${BINARIES_DIR}/start-qemu.sh"

if [[ "${DEFCONFIG_NAME}" =~ ^"vvoronin_*" ]]; then
    # Not a vvoronin defconfig, can't test.
    exit 0
fi
...
QEMU_CMD_LINE="$(sed -r ':a; /\\$/N; s/\\\n//; s/\t/ /; ta; /# '"qemu_aarch64_virt_defconfig"'$/!d; s/#.*//' ${README_FILES})"
...
sed -e "s|@SERIAL_ARGS@|${SERIAL_ARGS}|g" \
    ...
    <"${VVORONIN_BOARD_DIR}/start-qemu.sh.in" \
    >"${START_QEMU_SCRIPT}"
